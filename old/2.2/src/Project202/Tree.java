...
public class Tree {
	...
	protected static void prn(String s){
...
//						if(p.children[j] != null)
	...
	protected String print(){		//in one line
		Node f;
		NodeFolder r = root;
		Queue<Node> q = new Queue<Node>();
		String x = new String();
...
//					if(r.children[i] != null)
	...
	protected String printBFT(){		//level by level
		Node f;
		NodeFolder r = root;
		Queue<Node> q = new Queue<Node>();
		String x = new String(">");
...
//					if(r.children[i] != null)
	...
	/*
	 * will not append open files
	 */
	protected boolean insertFile(String h, String hj)throws GreaterThanMaxException, IOException, FileNotFoundException{
		if(searchPath(h)){			//child exists
			return false;
		}
		else if(!h.contains("\\")){
			return false;
		}
		else if(!searchPath(h.substring(0,h.lastIndexOf("\\")))){
			return false;
		}
		new File(h).createNewFile();
		PrintWriter out = new PrintWriter(new FileWriter(h));
		out.append(hj);
		out.close();
		scan();
		return true;
